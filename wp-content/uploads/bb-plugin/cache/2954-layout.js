...
{var field=$(this);field.removeClass('fl-form-error');field.siblings('.fl-form-error-message').hide();}};$(function(){FLBuilderLayout.init();});})(jQuery);(function($){$(function(){$('.fl-embed-video').fitVids();});})(jQuery);(function($){$(function(){if(typeof $.fn.magnificPopup!=='undefined'){$('.fl-node-588c06c1c91f9 .fl-mosaicflow-content, .fl-node-588c06c1c91f9 .fl-gallery').magnificPopup({delegate:'.fl-photo-content a',closeBtnInside:false,type:'image',gallery:{enabled:true,navigateByImgClick:true,},'image':{titleSrc:function(item){}},callbacks:{open:function(){if(this.items.length>0){var parent,item,newIndex=0,newItems=[];$(this.items).each(function(i,data){item=$(this);if('undefined'!==typeof this.el){item=this.el;}
...
$('.fl-node-588c06c1c91f9 .fl-mosaicflow-content').one('filled',function(){var hash=window.location.hash.replace('#','');if(hash!=''){FLBuilderLayout._scrollToElement($('#'+hash));}
if('undefined'!=typeof Waypoint){Waypoint.refreshAll();}}).mosaicflow({itemSelector:'.fl-mosaicflow-item',columnClass:'fl-mosaicflow-col',minItemWidth:300});});})(jQuery);(function($){FLBuilderPostGrid=function(settings)
...
{if('load_more'==this.settings.pagination&&this.totalPages==this.currPage){$(this.nodeClass+' .fl-builder-pagination-load-more').remove();}}};})(jQuery);(function($){$(function(){new FLBuilderPostGrid({id:'588c06378a41e',layout:'feed',pagination:'none',postSpacing:'60',postWidth:'300',matchHeight:0});});})(jQuery);